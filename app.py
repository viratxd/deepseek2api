...
async def chat_completions(request: Request):
    try:
        ...
        model_config = {
            ...
            "deepseek-v3-search": (False, True),
            "deepseek-chat-search": (False, True),
            "deepseek-r1-search": (True, True),
            "deepseek-reasoner-search": (True, True)
        }
        thinking_enabled, search_enabled = model_config.get(model_lower, (None, None))
        ...
        stream_mode = bool(req_data.get("stream", False))
        ...
        if stream_mode:
            if deepseek_resp.status_code != 200:
                ...
                deepseek_resp.close()
                return JSONResponse(content=content, status_code=deepseek_resp.status_code)

            def sse_stream():
                # ... (保持原有的流式处理逻辑不变)
                # [保持原有的流式处理代码，此处省略以节省空间]
